...
export function useWindowControlsPadding(role: WindowControlsPaddingRole): {
  ...
  const raw = {
    left: obstruction.topLeft?.width ?? 0,
    right: obstruction.topRight?.width ?? 0,
    top: Math.max(obstruction.topLeft?.height ?? 0, obstruction.topRight?.height ?? 0),
  };

  return useMemo(() => {
    if (role === "sidebar") return { left: raw.left, right: 0, top: raw.top };
    if (role === "header") {
      return { left: sidebarOpen ? 0 : raw.left, right: explorerOpen ? 0 : raw.right, top: 0 };
    }
    if (role === "detailHeader") return { left: 0, right: raw.right, top: 0 };
    if (role === "tabRow") {
      return {
        left: focusModeEnabled ? raw.left : 0,
        right: focusModeEnabled ? raw.right : 0,
        top: 0,
      };
    }
    return { left: 0, right: raw.right, top: raw.top };
  }, [explorerOpen, focusModeEnabled, raw.left, raw.right, raw.top, role, sidebarOpen]);
}
