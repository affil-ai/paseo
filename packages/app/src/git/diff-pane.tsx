...
export function DiffFilesPane({
  ...
  const collapsedFoldersArray = usePanelStore((state) =>
    stateKey ? state.diffCollapsedFoldersByWorkspace[stateKey] : undefined,
  );
  ...
  const handleToggleViewMode = useCallback(() => {
    ...
    if (nextViewMode === "tree") {
      ...
      if (stateKey) {
        setDiffCollapsedFoldersForWorkspace(stateKey, []);
      }
    }
    void updateChangesPreferences({ viewMode: nextViewMode });
  }, [setDiffCollapsedFoldersForWorkspace, stateKey, updateChangesPreferences, viewMode]);
  ...
  const handleToggleFolder = useCallback(
    (dirPath: string) => {
      ...
      setDiffCollapsedFoldersForWorkspace(stateKey, nextCollapsed);
    },
    [collapsedFolders, computeItemOffset, setDiffCollapsedFoldersForWorkspace, stateKey],
  );
  ...
  }, [allFileDiffsExpanded, files, setDiffExpandedPathsForWorkspace, stateKey]);
      ...
      <View style={styles.diffStatusInner}>
        ...
        <View style={styles.diffStatusButtons}>
          ...
          <DiffOptionsMenu
            ...
            overflowToggleStyle={overflowToggleStyle}
            refreshSupported={Boolean(refresh)}
            showWhitespaceToggle={showWhitespaceToggle}
            wrapLines={wrapLines}
            onRefresh={refresh?.onRefresh ?? noop}
