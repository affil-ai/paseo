...
function buildClassifierPrompt(email: ResendReceivedEmail): string {
  return [
    ...
    "Prioritize the newest human-written message above forwarded or quoted content.",
    "Do not classify an email as non-support solely because it is forwarded or includes a receipt, notification, screenshot, or automated email.",
    "If a human asks for help, asks a question, reports a problem, or requests a refund/cancellation/lookup, classify it as support even when the evidence is a forwarded receipt or notification.",
    "When uncertain, prefer support so a person can review it.",
    "",
    "Support includes:",
    "- customer questions",
    ...
    "- forwarded receipts or notifications with no human-written request for help",
    "",
    "Return JSON only:",
