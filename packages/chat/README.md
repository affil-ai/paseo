...
To archive the office agent and unlink its thread, send the exact standalone command `done` (for example, `@cto done`) or `archive`. Matching ignores case, surrounding whitespace, and the stripped bot mention, but does not match prose such as `archive this` or `done?`. The legacy `/archive` command remains compatible, although Slack may intercept it as an unsupported slash command in threads before the bridge receives it.
