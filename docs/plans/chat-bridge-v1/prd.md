...
- **Teardown** uses exact standalone `done` (for example, `@cto done`) or `archive`; legacy
  `/archive` remains compatible but Slack may intercept it in threads. These commands are the office
  brain's capture trigger.
